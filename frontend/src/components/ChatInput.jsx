...
const ChatInput = () => {
  ...
  const toggleMic = () => {
    if (!recognitionRef.current) alert("Speech recognition not supported");
  ...
  const handleSendMessage = async () => {
    ...
    if (conversation.title?.toLowerCase() === "new chat") {
      const conv = await updateConversation({
        id: conversation._id,
        title: value.trim().slice(0, 20),
      });
      dispatch(
        setConversationTitle({
      ...
      >
              ...
              {selectedFile?.type === "application/pdf" ? (
                ...
                selectedFile?.type.startsWith("image/") && (
                  <img
                    ...
                    className="h-10 w-10 rounded0xl object-cover mt-3"
                  />
                )
              )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            hover:text-slate-400 hover:bg-white/[0.05] border border-transparent hover:broder-white/[0.06]
            ...
            <button
              className={`flex items-center justify-center w-8 h-8 rounded-lg  border border-transparent hover:broder-white/[0.06]
          ...
          <button
            ...
            className={`flex items-center justify-center w-8 h-8 rounded-lg border-none
