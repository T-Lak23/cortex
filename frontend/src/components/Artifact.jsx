...
const Artifact = () => {
    ...
    return (
      <>
        ...
        {!collapsed ? (
          <div className="flex flex-col h-full bg-[#0d0f14]">
            <div className="h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
              <button
                className="
        flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200
        ...
        "
            ...
            bg-indigo-500/10 border border-indigo-500/20 shrink-0
            "
                >
          ...
          text-slate-400 hover:text-slate-200 hover:bg-white/[0.05] rounded-lg transition-colors
          duration-150 bg-transparent border-nonw cursor-pointer
          "
                  onClick={() => handleCopy()}
                >
                ...
                {canPreview && (
                  <div className="flex items-center gap-1 bg-white/[0.04] border border-white/[0.06] p-3 rounded-lg">
                    <button
                      onClick={() => setTab("code")}
                      className={`
                  flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md transition-colors duration-150
            ...
            {tab === "code" && (
              <div
                className="h-auto flex border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none]
            ...
            "
              >
                ...
                duration-150 border-r border-white/[0.05] relative cursor-pointer bg-transparent
                ${activeFile === index ? "text-indigo-400" : "text-slate-500 hover:text-slate-300"}
                `}
                  >
                    {f?.name}
                    {activeFile === index && (
                      <div className="absolute bottom-0 left-0 h-[2px] bg-indigo-500 rounded-t-full" />
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="hidden h-full lg:flex border-1 border-white/[0.06] bg-[#0d0f14] flex-col items-center py-4 gap-3 shrink-0">
            <button
              className="
        flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200
        hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none
  ...
  return (
    ...
    bg-indigo-600 hover:bg-indigo-500 text-white text-[12px] font-medium shadow-lg
    shadow-indigo-500/20 border-none cursor-pointer transition-colors duration-150
    "
      >
        <Code2 size={13} />
        View Code
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed inset-y-0 right-0 z-50 w-[88vw] max-w-[420px]
              border-1 border-white/[0.06] overflow-hidden
      ...
      <motion.div
        ...
        className="hidden lg:flex h-full border-1 border-white/[0.06]
